...
function todayWIB() {
  // Pakai tanggal lokal browser (WIB user)
  const now = new Date();
...
// Kembalikan tanggal sehari sebelumnya
...
export default function App() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      // Query 2 tanggal sekaligus (hari ini + kemarin UTC) supaya match tengah malam tidak hilang
      const dates = [prevDate(date), date];

      const fixturePromises = selLeagues.map(leagueId => {
        return Promise.all(
          dates.map(d =>
            fetch(`/api/football?endpoint=fixtures&league=${leagueId}&date=${d}`)
              .then(r => r.json())
      ...
      // Standings tidak butuh season
      const standingPromises = selLeagues.map(leagueId => {
        return fetch(`/api/football?endpoint=standings&league=${leagueId}`)
          .then(r => r.json())
          .then(d => {
      ...
      // Gabung semua fixtures
      const allFlat = allFixtures.flat();

      // Filter hanya yang masuk tanggal WIB yang dipilih user
      const wibStart = new Date(date + "T00:00:00").getTime(); // midnight lokal
      const wibEnd   = new Date(date + "T23:59:59").getTime(); // end of day lokal
      const merged = allFlat
        .filter(f => {
          const t = new Date(f.fixture.date).getTime();
          return t >= wibStart && t <= wibEnd;
        })
      ...
      setDebug({
        queriedDates: dates,
        rawTotal: allFlat.length,
        afterFilter: merged.length,
        leaguesQueried: selLeagues,
      });
  ...
  return (
    <div style={{ fontFamily: "system-ui, Arial, sans-serif", background: "#050d17", minHeight: "100vh", color: "#e2e8f0" }}>
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "12px 14px" }}>
        ...
        {/* Debug */}
        {debug && (
          <div style={{ background: "#0a1628", border: "1px solid #1e3a5f", borderRadius: 8, padding: 10, marginBottom: 12, fontSize: 10, color: "#60a5fa", lineHeight: 1.8 }}>
            <div style={{ fontWeight: 700, marginBottom: 4 }}>🔍 DEBUG INFO</div>
            <div>📅 Tanggal di-query ke API: <b>{debug.queriedDates?.join(" + ")}</b></div>
            <div>📦 Total fixture dari API: <b>{debug.rawTotal ?? debug.afterFilter}</b></div>
            <div>✅ Setelah filter WIB: <b>{debug.afterFilter}</b></div>
            <div>🌍 Liga: <b>{debug.leaguesQueried?.join(", ")}</b></div>
          </div>
        )}
