...
PENTING: Balas HANYA dengan raw JSON saja. JANGAN tambah penjelasan, JANGAN pakai markdown, JANGAN pakai backtick. Langsung mulai dengan { dan akhiri dengan }:
{
  ...
  // Jika JSON terpotong, tutup manual
  if (!jsonStr.includes("}")) jsonStr += \`,"summary":"Data tersedia","verdict":"BORDERLINE","stake":"1u"}\`;
  
  // Ambil JSON yang valid (cari closing brace terakhir)
  let depth = 0, end = -1;
  for (let i = 0; i < jsonStr.length; i++) {
    if (jsonStr[i] === "{") depth++;
    else if (jsonStr[i] === "}") { depth--; if (depth === 0) { end = i; break; } }
  }
  if (end === -1) jsonStr = jsonStr + "}";
  else jsonStr = jsonStr.slice(0, end + 1);

  let result;
  try {
    result = JSON.parse(jsonStr);
  } catch(e) {
    // Last resort: coba parse parsial
