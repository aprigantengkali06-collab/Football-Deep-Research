...
async function fetchAnalysis(home, away, leagueName, lcc) {
  const prompt = `Analisis pertandingan: ${home} vs ${away} (${leagueName}). LCC=${lcc}.
Berikan prediksi lengkap. Balas JSON saja tanpa teks lain:
{"hF":"WWDLW","aF":"LWDWL","hR":2,"aR":5,"hP":58,"aP":42,"h2h":"singkat","inj":"singkat","ctx":"singkat","swp":72,"fr":81,"ff":58,"v":"BORDERLINE","o25":68,"stk":"1u","sum":"ringkasan 1 kalimat","ah":"-0.5","ahConf":"TINGGI","ftOU":"Over 2.5","ftOUConf":"SEDANG","htOU":"Under 1.5","htOUConf":"TINGGI","ftBTTS":"No","htBTTS":"No","ftResult":"Home Win","htResult":"Draw"}`;
...
export default function App() {
  ...
  const fetchLeague = useCallback(async (league) => {
    ...
    try {
      ...
      const [liveRes, nextRes, todayRes, tomorrowRes] = await Promise.all([
        fetch(`/api/football?endpoint=fixtures&live=all&league=${league.id}`).then(r=>r.json()),
        fetch(`/api/football?endpoint=fixtures&league=${league.id}&next=20`).then(r=>r.json()),
        fetch(`/api/football?endpoint=fixtures&league=${league.id}&date=${today}`).then(r=>r.json()),
        fetch(`/api/football?endpoint=fixtures&league=${league.id}&date=${tomorrow}`).then(r=>r.json()),
      ]);

      const liveF    = liveRes.response||[];
      const nextF    = nextRes.response||[];
      const todayF   = todayRes.response||[];
      const tomorrowF= tomorrowRes.response||[];
      const seen = new Set();
      const merged = [...liveF,...nextF,...todayF,...tomorrowF].filter(f=>{
